...
if [ -n "$jellyfin_path" ]; then
  items_json="$(curl -fsS "${JELLYFIN_URL}/Items?parentId=${JELLYFIN_LIBRARY_ID}&recursive=true&includeItemTypes=Series&fields=Path&api_key=${JELLYFIN_API_KEY}")" || exit 1
